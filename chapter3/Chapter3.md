...
次の3通りの入力について，3種類のボールを買う値段が **安い方** のショップを出力してください．  
...
<details>
  <summary>ヒント</summary>
  <p>aとbの組合せを，Aが勝つ場合，Bが勝つ場合，引き分けの場合の3通りに分けてみましょう</p>
</details>
...
`random`モジュールの`random`メソッドは，0.0から1.0未満の数をランダムに返します．  
`random`メソッドの出力をグー(= 0)，チョキ(= 1)，パー(= 2)にうまく合わせることで，CPUの手をランダムに出させることができます．  
...
<details>
    <summary>ヒント</summary>
    <p>区間 `[0.0, 1.0)` のランダムな値を3倍することで区間 `[0.0, 3.0)` のランダム値を作ることができます</p>
</details>
