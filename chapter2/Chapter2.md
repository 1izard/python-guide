...
Python では数値，文字列，データ構造，関数といったあらゆるものが **オブジェクト** として実装されています．  
今まで見てきた int 型 や float 型，bool 型，str 型の値もオブジェクトです．  
オブジェクトとは物体とか具体物とかを指す言葉ですが，ここでは "値だけでなくメソッドなどいろいろな情報を保持しているもの" と考えてください．  
**メソッド** とは，オブジェクトが持っている便利な機能のことです (Extra) Python ではクラスのメンバーである変数は **属性** (attribute)，関数はメソッドと呼ばれます．(ここまでExtra)  
